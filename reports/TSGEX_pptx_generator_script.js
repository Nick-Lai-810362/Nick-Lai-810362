...
/* ============ SLIDE 9: OKX DUAL INVESTMENT ============ */
{
  ...
  card(s, 0.6, 5.75, PW - 1.2, 1.15, "財務運用場景",
    "① 設定目標價逢低分批建倉：於等待建倉價位期間同時賺取權利金，等同「限價單＋額外收益」的複合工具｜② 震盪市掛深度價外（OTM 10%+）收益增強：降低轉換機率，以較低年化換取較高本金安全邊際",
...
{
  const s = newSlide();
  sectionTitle(s, "④ 進階操作：滾動掛單策略，提高解套後之勝率", "本金轉為 BTC 後持續以同一行使價反向掛「高賣」收取權利金，等待價格回升");
  ...
  statCallout(s, 0.6, 3.95, 3.9, 1.55, "+2.05%（3週）", "本例路徑：本金全數取回＋累積權利金 $20,535", { valueSize: 22, valueColor: GOOD });
  statCallout(s, 4.7, 3.95, 3.9, 1.55, "-28.4%（未實現）", "對照情境：10週價格未回升，帳面浮虧 $284,436", { valueSize: 22, valueColor: WARN, fill: LIGHT_TINT2 });
  card(s, 8.75, 3.95, 3.98, 1.55, "執行紀律",
    "只要維持同一（或更高）行使價滾動、不因情緒而任意下修，價格回升後即保證取回本金加計期間權利金。",
    { fill: LIGHT_TINT2, bodySize: 10.5 });

  card(s, 0.6, 5.7, PW - 1.2, 1.2, "配套治理措施",
    "① 建議搭配停利機制（見下頁），可顯著提升整體年化並大幅降低虧損機率｜② 資金應規劃為獨立風險預算，不佔用日常營運流動性｜③ 依 FVTPL 原則按季揭露未實現評價，確保財務透明",
    { fill: LIGHT_TINT, bodySize: 10.5 });
  footer(s, "10 / 18");
}

/* ============ SLIDE 9C: MONTE CARLO FULL-YEAR SIMULATION (v2: corrected calibration + decomposition) ============ */
{
  const s = newSlide();
  sectionTitle(s, "④ 全年模擬：勝率驗證與治理重點", "蒙地卡羅模擬（20,000條路徑）｜現貨$77,000｜年化波動55%｜權利金已校準至125%(ATM)，非武斷假設");

  stdTable(s, [
    ["", "規則1：最高收益(ATM)", "規則2：不被轉換(OTM緩衝)"],
    ["【已平倉】占比 ／ 平均年化 ／ 虧損機率", "5.5% ／ +38.5% ／ 0%", "8.1% ／ +16.0% ／ 0%"],
    ["【未平倉，年末快照】占比 ／ 平均年化", "94.5% ／ -20.5%", "92.0% ／ -28.3%"],
    ["混合平均年化（原始揭露方式）", "-17.2%", "-24.7%"],
  ], { x: 0.6, y: 1.75, w: 8.3, colW: [3.5, 2.4, 2.4], fontSize: 9.5, rowH: 0.85 });

  statCallout(s, 9.05, 1.75, 3.68, 1.6, "已平倉路徑\n100%正報酬", "只要完整走完一輪，必然是正報酬——驗證了機制設計的本質", { valueSize: 18, valueColor: GOOD });
  card(s, 9.05, 3.55, 3.68, 2.15, "關鍵治理重點：週期長度",
    "5.5%~8.1%的路徑能在一年內走完並實現獲利；其餘部位年末仍在持有中，代表這是一個「存續期間不固定」的部位，而非「已虧損」——需要的是主動管理與時間規劃，而非視為零風險之現金管理工具。",
    { fill: LIGHT_TINT, bodySize: 10.5 });

  card(s, 0.6, 4.15, 8.3, 1.55, "為什麼週期長度不固定？",
    "零漂移（公平隨機漫步）下，「價格回到某個固定點」所需的時間在數學上沒有上限——這是隨機漫步的古典性質，非策略設計缺陷。若解套後採取「停利、不再滾動」的紀律（見下頁），可大幅縮短平均曝險時間、顯著改善整體結果。",
    { fill: LIGHT_TINT2, bodySize: 10.5 });

  card(s, 0.6, 5.85, PW - 1.2, 1.0, "財務部審查建議",
    "本策略勝率高、機制設計上保證已平倉部位不低於本金；審查重點應放在「如何管理存續期間」（部位規模、停利紀律、FVTPL季度評價揭露），而非單看混合平均數字。",
    { fill: LIGHT_TINT, bodySize: 10.5 });
  footer(s, "11 / 18");
}

/* ============ SLIDE 9C2: TAKE-PROFIT DISCIPLINE (the actionable positive finding) ============ */
{
  const s = newSlide();
  sectionTitle(s, "④ 建議做法：解套後停利了結，不做永續滾動", "同一套模擬，加上一條紀律——解套轉回USDT後即停止重新掛單、資金轉為現金——結果明顯改善");

  stdTable(s, [
    ["", "規則1：最高收益(ATM)", "規則2：不被轉換(OTM緩衝)"],
    ["中位數年化報酬", "+3.87%", "+1.89%"],
    ["全年虧損機率", "20.4%", "18.9%"],
    ["對照：永續滾動之虧損機率", "74.5%", "85.1%"],
  ], { x: 0.6, y: 1.75, w: 7.6, colW: [3.0, 2.3, 2.3], fontSize: 10.5, rowH: 0.7 });

  statCallout(s, 8.35, 1.75, 4.38, 1.75, "虧損機率降低逾50個百分點", "單一紀律：停利後不重新進場", { valueSize: 19, valueColor: GOOD });

  card(s, 0.6, 3.75, 7.6, 1.85, "為什麼有效？",
    "永續滾動策略之所以表現不佳，並非機制本身有缺陷，而是「一解套就立刻用新的市價重新掛單」使資金反覆暴露於套牢風險。只要在解套當下停手、把資金轉回現金，就能保留已實現之獲利、避免立即再次陷入下一輪不確定的存續期間。",
    { fill: LIGHT_TINT, bodySize: 11.5 });

  card(s, 8.35, 3.75, 4.38, 1.85, "代價",
    "停利後之閒置資金不再產生權利金收入，且平均每年仍會遇到約 1 至 2 成之路徑於年底前尚未解套。",
    { fill: LIGHT_TINT2, bodySize: 11 });

  card(s, 0.6, 5.75, PW - 1.2, 1.15, "落地建議",
    "將本策略之標準作業程序（SOP）明訂為「解套後停利、待下一次進場訊號或固定週期再重新申購」，而非交易所預設之「自動再訂閱」；此為單一、低成本即可落實之治理措施，且已由模擬證實可大幅改善整體風險報酬輪廓。",
    { fill: LIGHT_TINT, bodySize: 11 });
  footer(s, "12 / 18");
}

/* ============ SLIDE 9D: TENOR ROBUSTNESS CHECK ============ */
{
  const s = newSlide();
  sectionTitle(s, "④ 穩健性檢驗：換一個天期，結論會不同嗎？", "天期非固定7天——依選擇權定價原理（權利金∝√天期），重新校準1～30天全區間並重跑模擬");

  stdTable(s, [
    ["天期", "每年週期數", "校準ATM年化", "混合平均年化", "已平倉占比／平均"],
    ["1 天", "365", "330.7%", "-19.0%", "2.0% ／ +42.3%"],
    ["3 天", "122", "190.9%", "-18.2%", "3.7% ／ +41.6%"],
    ["7 天（原基準）", "52", "125.0%", "-17.0%", "5.5% ／ +38.3%"],
    ["14 天", "26", "88.4%", "-16.3%", "7.9% ／ +35.5%"],
    ["30 天", "12", "60.4%", "-14.6%", "11.9% ／ +32.8%"],
  ], { x: 0.6, y: 1.75, w: PW - 1.2, colW: [2.2, 2.1, 2.2, 2.2, 2.6], fontSize: 10, centerCols: [1, 2, 3, 4], rowH: 0.55 });

  statCallout(s, 0.6, 5.1, 3.9, 1.55, "-14.6% ~ -19.0%", "全天期範圍內，混合平均年化僅小幅波動", { valueSize: 20, valueColor: WARN });
  card(s, 4.7, 5.1, 3.9, 1.55, "反直覺發現",
    "天期越長，已平倉占比反而越高（30天11.9% vs 1天2.0%）——長天期每次檢查點有更充裕空間讓價格越過鎖定價，短天期雖單期年化最亮眼，但檢查過於密集反而更快再度套牢。",
    { fill: LIGHT_TINT, bodySize: 10 });
  card(s, 8.7, 5.1, 4.03, 1.55, "結論",
    "換任何天期，「已平倉必為正、但完成比例偏低」的核心結論都不變——不是7天這個假設剛好湊出負值。",
    { fill: LIGHT_TINT2, bodySize: 10.5 });
  footer(s, "13 / 18");
}
...
/* ============ SLIDE 13: ALLOCATION MODELS ============ */
{
  ...
  card(s, 0.6, 5.1, PW - 1.2, 1.6, "方法論揭露（重要）",
    "雙幣理財之 45% 年化係採「未觸及行使價（價外到期）」情境計算，未反映觸價本金轉換損失。若納入機率加權情境分析，實際綜合年化將視市場波動與觸價機率顯著偏離本表數字；惟第 11–12 頁之全年模擬顯示，只要搭配停利紀律（解套後不永續滾動），虧損機率可由七至八成大幅降至二成以下，詳見報告第 6、7 章完整情境試算。",
    { fill: LIGHT_TINT, bodySize: 11 });
  footer(s, "17 / 18");
}
