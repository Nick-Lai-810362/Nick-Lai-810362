...
{
  const s = newSlide();
  sectionTitle(s, "④ 全年模擬：為何「每期都正報酬」全年卻可能是負的？", "蒙地卡羅模擬（20,000條路徑）｜現貨$77,000｜年化波動55%｜權利金已校準至125%(ATM)，非武斷假設");
  ...
  statCallout(s, 9.05, 1.75, 3.68, 1.6, "已平倉路徑\n100%正報酬", "驗證：只要完整走完一輪，必然是正報酬——與直覺一致", { valueSize: 18, valueColor: GOOD });
  card(s, 9.05, 3.55, 3.68, 2.15, "但只有5.5%~8.1%能在一年內走完",
    "其餘九成以上路徑年末仍持有BTC，「負報酬」幾乎全來自這批未平倉部位在年末當天的市價快照，而非策略已實現虧損。",
    { fill: LIGHT_TINT, bodySize: 10.5 });

  card(s, 0.6, 4.15, 8.3, 1.55, "為什麼「未平倉」的比例這麼高？",
    "零漂移（公平隨機漫步）下，「價格回到某個固定點」所需的期望時間在數學上是無窮大——這不是模型瑕疵，是隨機漫步的古典性質。且兩規則都要求「一解套就立刻重新掛單」，新倉位又有相當機率馬上再度套牢，因此「卡住」才是這套規則長期下的常態，而非過渡。",
    { fill: LIGHT_TINT2, bodySize: 10.5 });

  card(s, 0.6, 5.85, PW - 1.2, 1.0, "財務部審查建議",
    "不應只看「混合平均年化」（易誤判為策略虧損），也不應只看「已平倉100%正報酬」（易誤判為零風險）；正確理解是：勝率高，但完成週期需要的時間沒有上限，期間資金以未平倉部位形式暴露於市價波動與FVTPL未實現損益認列。",
    { fill: LIGHT_TINT, bodySize: 10.5 });
  footer(s, "11 / 17");
}
